...
Группы, `VK_DEVICE_ID` и сервисный ключ для расписания положи в файл `memory/local.env` (его нет в git). Три строки: имя переменной, равно, значение. Не коммитить и не слать в чат.

По расписанию на **этом ПК** (не облако Cursor): Планировщик заданий Windows → каждые 30–60 мин → файл `scripts/run-live.ps1`. Компьютер должен быть включён. Облако из‑за чужого IP пока не ставить на автозапуск.
